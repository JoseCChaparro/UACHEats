...
router.get('/', (req, res) => {
    if (req.isAuthenticated()) {
        res.render("index", { logged: true });//antes index
    } else {
...
router.get('/menu', checkAuth, (req, res) => {
    // adding a new parameter for checking verification
    if (req.isAuthenticated()) {
        res.render("menu", { logged: true, username: req.user.username, verified : req.user.isVerified});
    } else {
        res.render("menu", { logged: false , username: req.user.username, verified : req.user.isVerified});
    }
